...
usersRouter.get('/', function (req, res) {
  ...
  session
    ...
    .then(function (result) {
      if (result.records.length == 0) res.send('No user with that username.');
      else {
...
usersRouter.get('/search', function (req, res) {
  ...
  session
    ...
    .then(function (result) {
      if (result.records.length == 0) res.send('No more users');
      else {
...
usersRouter.post('/createAccount', function (req, res) {
  ...
  session
    .run(
      `CREATE (u:User {name:'${name}', username: '${username}', password: '${pass}'})`
    )
    .then(function (result) {
...
usersRouter.post('/follow', function (req, res) {
  ...
  session
    ...
    .then(function (result) {
      ...
      else res.send(`${username} now follows ${follow}`);
    })
    .catch(function (err) {
      res.send(err);
    });
...
usersRouter.post('/unfollow', function (req, res) {
  ...
  session
    ...
    .then(function (result) {
      if (result.records.length == 0)
        res.send(
          'Invalid request. Wrong username provided or relationship does not exist'
        );
      else res.send(`${username} stopped following ${unfollow}`);
    })
    .catch(function (err) {
      res.send(err);
    });
...
gamesRouter.get('/', function (req, res) {
  ...
  session
    ...
    .then(function (result) {
      if (result.records.length == 0) res.send('No more games');
      else {
...
orderByLikes = function (gamesArr) {
  ...
  let finalArr = sortedIds.map(function (curr) {
    ...
    allTypesArray.forEach(function (current) {
      if (current.id == curr) game = current;
    });
    return game;
  });
